...
class Foot :
	...
	def moveSpeedRW(self, id, loc) :
		if (
			...
			spd = self.time(loc) #make valid time
...
#		self.moveSpeedRW(10*self.legNr+2, loc[1])
	...
	def rest(self) :
		loc = [0,0,0]
		if self.legNr == 2 or self.legNr == 5 :
			loc[0] = 0 
			loc[1] = 90 #40.68
			loc[2] = -45 #150 #120
		elif self.legNr == 1 or self.legNr == 4 :
			loc[0] = 45 
			loc[1] = 90 #40.68
			loc[2] = -45 #150 #120
		elif self.legNr == 3 or self.legNr == 6 :
			loc[0] = -45 
			loc[1] = 90 #40.68
			loc[2] = -45 #150 #120
	...
	def stepFour(self) :
		...
		if legNr == 1 : 		
			...
			loc[1] = -34.86
			loc[2] = -103.62
	moveSpeedRW(10*legNr+1, loc[0])
	moveSpeedRW(10*legNr+2, loc[1]) 
	moveSpeedRW(10*legNr+3, loc[2]) 
...
#		sleep(0.1)
		...
		while steps :
			...
			for leg in feet:
				if(leg.legNr%2 == 0) : leg.stepOne()
				else : leg.stepThree()
			ax.action()
			sleep(0.5)
			
			for leg in feet:
				if(leg.legNr%2 == 0) : leg.stepTwo()
				else : leg.stepFour()
			ax.action()
			sleep(0.1)
