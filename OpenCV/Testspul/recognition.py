...
class Recognition:
	...
	def targetBalloon(self, frame):
		...
		lower_red = np.array([0, 100, 60])#[0, 50, 100]
		upper_red = np.array([10, 255, 255])
		mask0 = cv2.inRange(hsv, lower_red, upper_red)
		lower_red = np.array([170,100, 60])#[170,50, 100]
		...
		for i in circles[0,:]:
			if radius < i[2]:
				lc = i
		radius = 0
		self.radiusTotal = self.radiusTotal + i[2]
		...
		if lastPosition is None:
			...
			lastPosition = None
                ...
                self.count += 1
		print "Median: " + (self.radiusTotal / count)
		print "Radius: " + lastPosition[2]
